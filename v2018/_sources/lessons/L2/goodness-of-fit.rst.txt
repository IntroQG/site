...
.. figure:: img/data-and-predictions.png
    :width: 500 px
    :align: center
    :alt: Data and predictions
...
where :math:`O_{i}` is the :math:`i`\ th measured mean age, :math:`E_{i}` is the :math:`i`\ th predicted age, and :math:`\sigma_{i}` is the :math:`i`\ th standard deviation.
...
2. If the prediction is exactly 1 standard deviation larger or smaller than the mean of the data, then the difference would be exactly the standard deviation :math:`\sigma`, in which case the chi-squared value for that data point would be :math:`\sigma^{2} / \sigma^{2}`, which equals 1.
...
where :math:`N` is the number of values in the summation.

The change above is simple, we're just dividing the calculated chi-squared value by the number of values used in the summation :math:`N`.
